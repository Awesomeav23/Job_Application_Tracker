...
async function setAction(action: 'NO ACTION' | 'RESTRICT') {
  await db.$executeRawUnsafe(`ALTER TABLE "analyses" DROP CONSTRAINT "analyses_documentId_fkey"`)
  await db.$executeRawUnsafe(
    `ALTER TABLE "analyses" ADD CONSTRAINT "analyses_documentId_fkey" ` +
...
async function main() {
  ...
  await fixture('a')
  results.directDelete = await attempt(
    'DELETE the resume that an analysis cites  (AI-003 must refuse this)',
    () => db.document.delete({ where: { id: 'd-a' } })
  )
  results.userDeleteNoAction = await attempt(
    'DELETE the user  (cascades to documents AND analyses at once)',
    () => db.user.delete({ where: { id: 'u-a' } })
  )
  ...
  await fixture('b')
  results.userDeleteRestrict = await attempt(
    'DELETE the same user, identical data, only the FK action differs',
    () => db.user.delete({ where: { id: 'u-b' } })
  )
